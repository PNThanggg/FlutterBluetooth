...
class BluetoothConnection {
  static const MethodChannel _channel = MethodChannel('com.example.flutter_bluetooth/bluetooth');

  // for only found a device broadcast receiver
  static const EventChannel _eventDeviceResultStreamChannel =
      EventChannel('com.example.flutter_bluetooth/deviceResultStream');

  // for only connecting and pairing status broadcast receiver for now
  static const EventChannel _eventStatusStreamChannel = EventChannel('com.example.flutter_bluetooth/anyStatusResultStream');

  // bluetooth is active or inactive, output: [true] or [false]
  Future<bool> isBluetoothAvailable() async {
    final bool isBluetoothAvailable = await _channel.invokeMethod('IS_BLUETOOTH_AVAILABLE');
  ...
  Future<void> connect({required String macAddress, String? uuidString}) async {
    try {
      await _channel.invokeMethod('CONNECT_TO_DEVICE', {'macAddress': macAddress, "UUIDString": uuidString});
    } catch (e) {
...
// location is active or inactive, output: [true] or [false]
  Future<bool> checkSettingLocation() async {
    var checkSettingLocation = await _channel.invokeMethod('IS_ON_LOCATION');
...
// goes to device's location setting for turn on
  ...
  Future<bool> applyPermissionLocation() async {
    var isPermissionLocation = await _channel.invokeMethod('APPLY_PERMISSION_LOCATION');
  ...
  //   var unpairAnswer = "";
  //   try {
  //     unpairAnswer = await _channel.invokeMethod('UNPAIR_TO_DEVICE', {'macAddress': macAddress});
  //   } catch (e) {
  //     print("Failed to pair: $e");
  //   }
  //
  ...
  // also you can give [macAddress] takes [macAddress] named parameter, type is [String]
  Future<void> disconnect({String? macAddress}) async {
    try {
      await _channel.invokeMethod('CLOSE_TO_DEVICE', {'macAddress': macAddress});
    } catch (e) {
      print("Failed to connect: $e");
    }
  }

  // takes two parameters one of parameters is [data], [data] is any text for now, type is [String]
  ...
  Future<void> write({String? data, bool? autoConnect}) async {
    try {
      await _channel.invokeMethod('WRITE_TO_DEVICE', {"data": data, "autoConnect": autoConnect});
    } catch (e) {
      print("Failed to connect: $e");
    }
  }
